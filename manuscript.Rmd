...
df <- df %>% 
  select(
    ...
    Extr:Trust, # composite variables for personality traits of perceivers
...
df <- 
  ...
  relocate(rapport_c, .before = rapport)
```
...
df_personality <-
  ...
  select(perceiver_id:group_id,
         ...
         InsecAtt:Trust, -ends_with("C"), -ASMtot, -beqtot, -SIAPS)
...
df_personality %>% 
  ...
  mutate(self_dist = target - self_norm,
         ...
         across(c(InsecAtt:Trust, self_norm:inf_dist), 
...
df_affect <- 
  ...
  pivot_longer(cols = target_affect1:judgment_affect120,
               names_to = c("affect", "item"),
               names_sep = "_",
               values_to = "rating") %>% 
  mutate(item = parse_number(item)) %>% 
  pivot_wider(names_from = "affect",
              values_from = "rating") %>% 
  right_join(., affect_norms, by = "item") %>% 
  mutate(self_dist = target - norm,
         across(norm:self_dist, scale, scale = FALSE, center = TRUE)
         )
