...
null_models %>% 
  kable_style(title = "<center><b>Null Models</b></center>") %>% 
...
by_model <- 
null_models %>% 
  mutate(model = c(rep("p", 5), rep("a", 5), rep("t", 5))
         ) %>% 
  group_by(model) %>% 
  nest()


map(by_model, specr::plot_variance)
...
map(null_models2, performance::icc, by_group = TRUE)
