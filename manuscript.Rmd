...
df_reverse <- 
  ...
  select(perceiver_id:group_id,
         negative_keyed_items)

df_reverse <- 
df_reverse %>% 
  mutate(across(all_of(negative_keyed_items), subtract_8)) %>% 
  rename_at(vars(negative_keyed_items), ~ paste0(negative_keyed_items, "rc"))

df <- left_join(df, df_reverse)
```
